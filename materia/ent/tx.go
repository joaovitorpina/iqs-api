...
func (tx *Tx) init() {
	tx.Categoria = NewCategoriaClient(tx.config)
	tx.Materia = NewMateriaClient(tx.config)
	tx.ProfissionalMaterias = NewProfissionalMateriasClient(tx.config)
}
...
// applies a query, for example: Categoria.QueryXXX(), the query will be executed
