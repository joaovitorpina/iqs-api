...
func (tx *Tx) init() {
	tx.Cep = NewCepClient(tx.config)
	tx.Cidade = NewCidadeClient(tx.config)
	tx.Endereco = NewEnderecoClient(tx.config)
	tx.Estado = NewEstadoClient(tx.config)
}
...
// applies a query, for example: Cep.QueryXXX(), the query will be executed
