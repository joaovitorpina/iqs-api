...
func (vq *VideoQuery) QueryProfissional() *ProfissionalQuery {
	query := &ProfissionalQuery{config: vq.config}
...
func (vq *VideoQuery) Clone() *VideoQuery {
	...
	return &VideoQuery{
		...
		withProfissional: vq.withProfissional.Clone(),
...
func (vq *VideoQuery) WithProfissional(opts ...func(*ProfissionalQuery)) *VideoQuery {
	query := &ProfissionalQuery{config: vq.config}
	for _, opt := range opts {
		opt(query)
	}
	vq.withProfissional = query
	return vq
}
...
func (vq *VideoQuery) sqlAll(ctx context.Context) ([]*Video, error) {
	var (
		...
		_spec       = vq.querySpec()
		loadedTypes = [1]bool{
			vq.withProfissional != nil,
		}
	)
	if vq.withProfissional != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := vq.withProfissional; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Video)
		for i := range nodes {
			if nodes[i].profissional_videos == nil {
				continue
			}
			fk := *nodes[i].profissional_videos
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "profissional_videos" returned %v`, n.ID)
			}
			for i := range nodes {
