...
func (tx *Tx) init() {
	tx.AreaSaude = NewAreaSaudeClient(tx.config)
	tx.Convenio = NewConvenioClient(tx.config)
	tx.Especializacao = NewEspecializacaoClient(tx.config)
	tx.Foto = NewFotoClient(tx.config)
	tx.Podcast = NewPodcastClient(tx.config)
	tx.Profissional = NewProfissionalClient(tx.config)
	tx.Tratamento = NewTratamentoClient(tx.config)
...
// applies a query, for example: AreaSaude.QueryXXX(), the query will be executed
